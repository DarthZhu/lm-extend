...
from src.models.modeling_qwen2_vl import Qwen2VLForConditionalGeneration
from src.models.modeling_llava_onevision import LlavaOnevisionForConditionalGeneration
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./output/head_detection/outputs/llava-onevision-qwen2-7b-si")
    parser.add_argument("--ask_twice", action="store_true")
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--num_sampling", type=int, default=1)
    
    parser.add_argument("--block_list", nargs='+', type=int)
    
    args = parser.parse_args()
    
    # initialize dataset, model, and tokenizer
    # model = Qwen2VLForConditionalGeneration.from_pretrained("data/models/llava-onevision-qwen2-7b-si", torch_dtype=torch.float16)
    model = LlavaOnevisionForConditionalGeneration.from_pretrained("llava-hf/llava-onevision-qwen2-7b-si-hf", torch_dtype=torch.float16)
    ...
    for layer_idx in range(28):
        for head_idx in range(28):
            block_list = [(layer_idx, head_idx)]
            print(f"Current block list: {block_list}")
            output_dir = args.output_dir
            if not os.path.exists(output_dir):
                os.mkdir(output_dir)
            output_path = os.path.join(output_dir, f"{args.dataset}_{json.dumps(block_list)}.prob")
    
            # inference
            progress_bar = tqdm(range(number_batch))
            for batch in batch_inputs[:-1]:
                batch_ids = [list(b.keys())[0] for b in batch]
                inputs = [list(b.values())[0] for b in batch]
                texts = [ipt["text"] for ipt in inputs]
                images = [ipt["image"] for ipt in inputs]
                batch_input = processor(text=texts, images=images, return_tensors="pt").to(model.device, torch.float16)
                
                outputs = model(
                    **batch_input,
                    # input_ids=batch_input.input_ids.to(model.device),
                    # attention_mask=batch_input.attention_mask.to(model.device),
                    # pixel_values=batch_input.pixel_values.to(model.device),
                    # image_grid_thw=batch_input.image_grid_thw.to(model.device),
                    block_list=block_list
                ).logits
                outputs = outputs.cpu()
                outputs = outputs[:, -1, :]
                target_tokens = ["A", "B", "C", "D"]
                target_token_ids = torch.tensor(processor.tokenizer.convert_tokens_to_ids(target_tokens))
                target_probs = torch.index_select(outputs, 1, target_token_ids)
                tokens = torch.argmax(target_probs, dim=-1)
                tokens = [target_tokens[token] for token in tokens]
                target_probs = target_probs.tolist()
                outputs = [[token, prob] for token, prob in zip(tokens, target_probs)]
