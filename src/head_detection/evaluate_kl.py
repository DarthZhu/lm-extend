...
def main():
    ...
    parser.add_argument("--folder", type=str, default="output/head_detection/outputs")
    ...
    ori_preds = read_pred_file("output/head_detection/outputs/mmmu_[].prob")
    
    results = {}
    kl_results = {}
    for layer_id in range(28):
        for head_id in range(28):
            file = os.path.join(args.folder, f"mmmu_[[{layer_id}, {head_id}]].prob")
            preds = read_pred_file(file)
            ...
            for data in datas:
                ...
                if pred is None:
                    continue
                scores = softmax(torch.tensor(pred[1]))
                pred = pred[0]
                ori_scores = softmax(torch.tensor(ori_preds.get(f"{_}")[1]))
                kl += kl_div(scores, ori_scores).item()
                ...
                if evaluate(answer, pred):
                    cnt_correct += 1
                cnt_valid += 1
            results.update({json.dumps((layer_id, head_id)): cnt_correct / cnt_valid})
            kl = kl / cnt_valid
            kl_results.update({json.dumps((layer_id, head_id)): kl})
            # print(f"All sample number: {cnt_valid}")
            # print(f"Correct sample number: {cnt_correct}")
            # print(f"Accuracy: {cnt_correct / cnt_valid}")
    with open("output/head_detection/accu_prob.json", "w") as fout:
        json.dump(results, fout, indent=2)
    with open("output/head_detection/kl.json", "w") as fout:
        json.dump(kl_results, fout, indent=2)
